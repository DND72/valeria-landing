  ...
  return new Promise((resolve, reject) => {
    execFile('python', [pythonScriptPath, birthDate, birthTime, city], (error: any, stdout: string, stderr: string) => {
      ...
      try {
        ...
        reject(new Error("Errore parsing output Python: " + stdout))
      }
    })
  })
}
...
async function test() {
  console.log("--- TEST VALERIA MAGNUM (BACKEND CONTEXT) ---")
  ...
  try {
    ...
    const { generateChartInterpretation } = await import('./src/lib/gemini.js')

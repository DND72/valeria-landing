...
export const calculateNatalChart = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (userId) {
      ...
      if (existing.rows.length > 0) return res.json({ ...existing.rows[0].chart_data, id: existing.rows[0].id, interpretation: existing.rows[0].interpretation, isExisting: true })
    }
...
export const generatePaidChart = async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    if (existing.rows.length > 0) return res.json({ ...existing.rows[0].chart_data, id: existing.rows[0].id, interpretation: existing.rows[0].interpretation, status: existing.rows[0].status, isExisting: true })
...
export const syncNatal = async (req: Request, res: Response) => {
    ...
    try {
      ...
      if (current.rows.length > 0 && current.rows[0].declared_birthday) {
         ...
         if (dbDate !== birthDate) return res.status(403).json({ error: 'birth_data_crystallized', message: 'Dati cristallizzati.' })
      }
...
export const calculateSynastry = async (req: Request, res: Response): Promise<void> => {
   ...
   const { personA, personB, isPreview } = req.body; try {
     ...
     if (isPreview) {
        ...
        if (wallet.rows[0]?.free_synastry_used) return res.status(403).json({ error: 'free_preview_exhausted' })
        ...
        return res.json({ status: 'ready', chartA, chartB, interAspects, interpretation: interp, is_preview: true })
     }
     ...
     if (exist.rows.length > 0) return res.json({ id: exist.rows[0].id, status: exist.rows[0].status, interpretation: exist.rows[0].status === 'ready' ? exist.rows[0].interpretation : null, isExisting: true })
     ...
     try {
       ...
       await dbClient.query('COMMIT'); res.json({ id: ins.rows[0].id, status: 'pending_staff' })
     } catch (e) { await dbClient.query('ROLLBACK'); throw e } finally { dbClient.release() }
   } catch (error) { res.status(500).json({ error: 'Internal error' }) }
}

export const getMyCharts = async (req: Request, res: Response): Promise<void> => {
  const userId = req.auth?.userId; if (!userId) return res.status(401).json({ error: "Non autorizzato" })
  try {
